...
const Expansion: React.FC = () => {
  return (
    <section id="new-habitat" className="relative py-16 md:py-32 bg-pop-dark text-white overflow-hidden">
       <div className="absolute inset-0 opacity-20 pointer-events-none" style={{ backgroundImage: 'linear-gradient(#444 1px, transparent 1px), linear-gradient(90deg, #444 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
               ...
               <div className="flex flex-col lg:flex-row gap-8 md:gap-12 items-center">
                   <div className="w-full lg:w-1/2 pt-4 md:pt-0">
                       <h2 className="text-3xl md:text-7xl font-display font-black uppercase mb-4 md:mb-6 leading-tight text-center md:text-left">
                           The Next <br/> <span className="text-pop-blue">Evolution</span>
                       </h2>
                       <p className="font-mono text-gray-400 mb-6 md:mb-8 text-xs md:text-lg text-center md:text-left">Project Code: EXPANSION_02</p>
                       <ul className="space-y-3 md:space-y-6">
                           {[
                               { title: "Centralized Kitchen", desc: "A 5,000 sqft innovation lab." },
                               { title: "Healthy Menu 2.0", desc: "Macros calculated. Flavor maximized." },
                               { title: "Gourmet Retail Range", desc: "Take the Habitat magic home." }
                           ].map((item, i) => (
                               <li 
                                  key={i}
                               ...
                               >
                                   ...
                                   <div>
                                       ...
                                       <p className="text-gray-400 text-[10px] md:text-sm">{item.desc}</p>
                                   </div>
                               </li>
                           ))}
                       </ul>
                   ...
                   <div className="w-full lg:w-1/2 relative">
                        <div className="relative aspect-square rounded-[1.5rem] md:rounded-[2rem] border-2 border-white/20 overflow-hidden group cursor-none transition-transform duration-500 hover:scale-[1.02] hover:-rotate-1" data-hover="true">
                             <img src="https://images.unsplash.com/photo-1590579491624-f98f36d4c763?q=80&w=2600&auto=format&fit=crop" className="w-full h-full object-cover grayscale opacity-50 group-hover:opacity-80 transition-opacity duration-500" alt="Blueprint" />
                             <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                 <div className="bg-pop-blue/90 p-4 md:p-6 rounded-full backdrop-blur-md border-2 border-white transform rotate-12 shadow-xl">
                                     <span className="font-display font-black text-lg md:text-2xl uppercase text-center block leading-tight">Coming<br/>Soon</span>
                                 </div>
                             </div>
                        </div>
                   </div>
