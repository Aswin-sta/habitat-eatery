...
const Navbar: React.FC = () => {
  ...
  return (
    <>
        ...
        <div className="pointer-events-auto">
            ...
            >
               <div className="relative w-5 h-5 md:w-6 md:h-6 flex items-center justify-center">
                 <Menu size={20} className={`absolute transition-all duration-300 md:w-6 md:h-6 ${isOpen ? 'opacity-0 scale-0' : 'opacity-100 scale-100'}`} />
                 <X size={20} className={`absolute transition-all duration-300 md:w-6 md:h-6 ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-0'}`} />
               </div>
            </button>
        </div>
      </header>
      ...
      >
         <nav className="relative z-10 flex flex-col items-center gap-4 md:gap-6">
