...
const FullMenu: React.FC<FullMenuProps> = ({ isOpen, onClose }) => {
    const [activeCategory, setActiveCategory] = useState<'all' | 'starter' | 'main' | 'dessert' | 'cocktail'>('all');
    ...
    return (
        ...
        >
            <div className="flex justify-between items-center mb-12 sticky top-0 bg-pop-cream/90 backdrop-blur-md py-4 z-20 border-b-2 border-black/10 px-6 md:px-12">
                ...
                <button onClick={onClose} className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-pop-pink border-2 border-black flex items-center justify-center hover:scale-110 transition-transform" data-hover="true">
            ...
            <div className="min-h-screen relative p-6 md:p-12">
                <div className="mb-12 flex flex-wrap gap-4">
                    {categories.map((cat) => (
                        <button key={cat} onClick={() => setActiveCategory(cat as any)} className={`px-6 py-2 md:px-8 md:py-3 rounded-full border-2 border-black font-bold uppercase tracking-wider text-xs md:text-sm transition-all ${activeCategory === cat ? 'bg-black text-white' : 'bg-white text-black hover:bg-pop-blue hover:text-white'}`} data-hover="true">{cat}</button>
                    ))}
                </div>
                    ...
                    {filteredItems.map((item) => (
                        ...
                        >
                            <div className="h-64 overflow-hidden border-b-4 border-black relative">
                                ...
                                <div className="absolute top-4 right-4 bg-pop-yellow px-4 py-1 border-2 border-black rounded-full font-bold text-sm shadow-md">${item.price}</div>
                            </div>
                            <div className="p-8">
                                <h3 className="text-3xl font-display font-black uppercase leading-none mb-4">{item.name}</h3>
                                <p className="font-sans font-medium text-gray-600 mb-6 line-clamp-2">{item.description}</p>
                                <div className="flex justify-between items-center mt-auto">
                                        <span className="text-xs font-bold uppercase tracking-wider bg-gray-100 px-3 py-1 rounded-lg">{item.category}</span>
                                        <button className="w-10 h-10 rounded-full bg-black text-white flex items-center justify-center hover:bg-pop-blue transition-colors group-hover:rotate-[-45deg] transition-transform duration-300"><ArrowRight size={18} /></button>
                                </div>
                            </div>
                        </div>
                    ))}
